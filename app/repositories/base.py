...
from config.values import NOT_MODIFIED, _NotModified
from config.db import DBModel, get_session
...
class DefaultModelRepository():
    ...
    async def create(self, **kwargs) -> DBModel:
        obj = self.model(**{key: self.safe_value(value) for key, value in kwargs.items()})
        self.session.add(obj)
        await self.session.commit()
        await self.session.refresh(obj)
        return obj

    async def get(self, id: str | UUID) -> DBModel | None:
        query = self.get_one_query(id)
        return (await self.session.execute(query)).scalar_one_or_none()

    async def get_page(self, page: int, page_size: int) -> tuple[list[DBModel], int]:
    ...
    async def update(self, id: str | UUID, **kwargs) -> DBModel | None:
        query = self.get_one_query(id)
    ...
    async def delete(self, id: str | UUID) -> DBModel | None:
        query = self.get_one_query(id)
        obj = (await self.session.execute(query)).scalar_one_or_none()
        if obj is None:
            return None
